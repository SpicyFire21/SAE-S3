<template>
  <div class="w-full relative">
    <button type="button" @click="scrollLeft" class="absolute left-0 top-1/2 transform -translate-y-1/2 bg-white/90 rounded-full p-3 shadow-lg z-10 hover:bg-white transition">
      <svg class="w-6 h-6 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
      </svg>
    </button>

    <div ref="carousel" class="flex overflow-x-hidden gap-8 scroll-smooth px-8">
      <div
          v-for="(provider, index) in providers"
          :key="provider.id || index"
          class="bg-white w-[calc(33.333%-16px)] flex-shrink-0 p-6 border border-gray-300 rounded-lg shadow-lg flex flex-col items-center"
      >
        <img v-if="provider.nom_photo" :src="getProviderImage(provider.nom_photo)" :alt="provider.name || 'Prestataire'" class="w-28 h-28 object-cover rounded-3xl mb-4"/>
        ...
        <div class="flex items-center mb-2 space-x-1">
          <template v-for="i in 5" :key="i">
            <svg v-if="i <= Math.floor(provider.note)" class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
            ...
            <svg v-else-if="i === Math.ceil(provider.note) && provider.note % 1 !== 0" class="w-5 h-5" viewBox="0 0 20 20">
              <defs>
        ...
        <a href="#" class="inline-flex items-center text-white tracking-widest bg-yellow-500 hover:bg-yellow-600 focus:ring-4 focus:ring-yellow-300 font-medium rounded-md text-sm px-4 py-2.5">
          {{ t('prestataireCard.giveOpinion') }}
        </a>
      </div>
    </div>

    <button type="button" @click="scrollRight" class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-white/90 rounded-full p-3 shadow-lg z-10 hover:bg-white transition">
      <svg class="w-6 h-6 text-gray-700" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const scrollAmount = () => carousel.value.offsetWidth / 3 + 24
