    ...
    <div ref="carousel" class="flex overflow-x-auto gap-4 scroll-smooth scrollbar-hide snap-x snap-mandatory">
      ...
      <div class="flex items-center mb-2 space-x-1">
        <template v-for="i in 5" :key="i">
          <svg v-if="i <= Math.floor(averageNoteByUser(provider.id))" class="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 20 20">
          ...
          <svg v-else-if="i === Math.ceil(averageNoteByUser(provider.id)) && averageNoteByUser(provider.id) % 1 !== 0" class="w-5 h-5" viewBox="0 0 20 20">
            <defs>
